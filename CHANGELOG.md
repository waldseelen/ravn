...
- **Linux packaging workflow** (`.github/workflows/linux-package.yml`) — `workflow_dispatch`-only, builds a PyInstaller onedir + `tar.gz` + SHA-256, bundles the Linux `yt-dlp` binary, and is written to smoke-test both the headless CLI and (under `xvfb`) the GUI, which is the one path the test matrix cannot cover since the suite never constructs a real Tk root. **Not yet executed on a real runner** — it is deliberately not wired into the tagged release, and a green manual run is the gate before Linux artifacts ship.
...
- **`yt-dlp`'s version never showed in Settings.** `tool_health` probed it with FFmpeg's single-dash `-version`; yt-dlp parses that as a bundle of short flags, prints its usage text, and still exits `0` — so the check "succeeded" and reported a blank version. Version flags are now per-tool.
- `build.ps1` wrote the downloaded `yt-dlp.exe` straight to its final path, so an interrupted download left a truncated binary that the next run would accept as already-bundled and ship. It now downloads to a temp file and only moves it into place once complete, matching how FFmpeg was already handled.
